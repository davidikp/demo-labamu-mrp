...
export const MaterialRequestDetailPage = ({ onNavigate, initialData, requestId, showSnackbar }) => {
  ...
  const handleConfirmTransfer = () => {
    ...
    // Notification: transfer started → notify requester to confirm receipt.
    notifyMr("material_request", "transfer_started", {
      entityId: request.id,
      requesterUser: mrUser,
      requesterName: request.requestedBy || mrUser.name,
      preparerName: request.preparedBy || "Inventory Team",
      workOrderNo: request.workOrderNo || request.woNo || "the linked work order",
    });
  };
  ...
  const handleConfirmCancel = () => {
    ...
    if (isRejectReceipt) {
      ...
      // Production POV rejected the receipt → notify the preparer.
      notifyMr("material_request", "receipt_rejected", {
        entityId: request.id,
        preparerUser: mrUser,
        preparerName: request.preparedBy || mrUser.name,
        requesterName: request.requestedBy || "Requester",
        reason: cancelReason.trim(),
      });
    } else {
      // Inventory POV (preparer) cancelled the request → notify the requester.
      notifyMr("material_request", "cancelled_by_preparer", {
        entityId: request.id,
        requesterUser: mrUser,
        requesterName: request.requestedBy || mrUser.name,
        preparerName: request.preparedBy || "Inventory Team",
  ...
  const handleConfirmReceipt = () => {
    ...
    // Notification: receipt confirmed → in-app to the preparer.
    notifyMr("material_request", "receipt_confirmed", {
      entityId: request.id,
      preparerUser: mrUser,
      preparerName: request.preparedBy || mrUser.name,
      requesterName: request.requestedBy || "Requester",
