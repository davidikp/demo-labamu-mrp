  ...
  const visibleOptions = searchable && searchTerm
    ? initialOptions.filter(option => {
        ...
        return (
          ...
          String(option.assignmentId || "").toLowerCase().includes(lowerSearch)
        );
      })
    : initialOptions;
...
/**
 * DropdownSelect — adapter over the ce-ui Dropdown for the common case, with a
 * fallback to the legacy implementation when a custom `renderOption` is supplied
 * (ce-ui Dropdown has no per-option render hook). The legacy cosmetic props
 * (fieldHeight, borderRadius, menuWidth, ...) are ignored by the ce-ui path,
 * which uses the design-system sizing instead.
 */
const SIZE_MAP = { sm: "sm", small: "sm", md: "md", medium: "md", lg: "lg", large: "lg" };
...
export { DropdownSelect, LegacyDropdownSelect };
